...
public class ViewOrder extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (request.getParameter("Order") != null && request.getParameter("Order").equals("ViewOrder")) {
            ...
            if (request.getParameter("orderId") != null && !request.getParameter("orderId").isEmpty()) {
                ...
                if (orderPayments.get(orderId) != null) {
                    for (OrderPayment od : orderPayments.get(orderId))
                        if (session.getAttribute("usertype").equals("retailer")) {
                            ...
                            if (od.getCustomername().equals(username))
                                size = orderPayments.get(orderId).size();
                        }
                }
                ...
                if (size > 0) {
                    ...
                    pw.print("<td>orderDate:</td>");
                    pw.print("<td>deliveryDate:</td>");
                    pw.print("<td>storeId:</td></tr>");
                    for (OrderPayment oi : orderPayments.get(orderId)) {
                        pw.print("<tr>");
                        pw.print("<td><input type='radio' name='orderName' value='" + oi.getOrderName() + "'></td>");
                        pw.print("<td>" + oi.getOrderId() + ".</td><td>" + oi.getCustomername() + "</td><td>"+ oi.getCustomer_address() + "</td><td>"+ oi.getProductType() + "</td><td>"
                                + oi.getQuantity() + "</td><td>"+ oi.getOrderName() + "</td><td> " + oi.getOrderPrice() +"</td><td>"
                                + oi.getPurchaseDate() + "</td>" + "</td><td>" + oi.getShipDate() + "</td><td>"+ oi.getStoreID() + "</td>");
                    ...
                    if (session.getAttribute("usertype").equals("customer")) {
                        ...
                        pw.print("<h4 style='color:red'>User has not placed any order with this order id</h4>");
                    }
                }
            } else {
            ...
            if (request.getParameter("orderName") != null) {
                ...
                for (OrderPayment oi : orderPayments.get(orderId)) {
                    if (oi.getOrderName().equals(orderName)) {
                        ...
                        if (currentDate.isBefore(oi.getShipDate().minusDays(5))) {
                            ...
                            Utilities.updateQuanity_increment(oi.getProductType(), oi.getProductId());
                        } else {
                            pw.print("<h4 style='color:red'>Your Order cannot be Cancelled</h4>");
                        }
                    }
                }
                ...
                for (OrderPayment oi : orderPayments.get(orderId)) {
                    if (oi.getOrderName().equals(orderName)) {
                        ...
                        if (currentDate.isBefore(oi.getShipDate().minusDays(2))) {
                            pw.print("<p> Your current Delivery/Pickup Date is : " +oi.getShipDate() +"</p>");
                            pw.print("<td> Update Delivery/Pickup Date: </td>");
                            pw.print("<td> <input type='date' name='shipmentDate'> </td>");
                            pw.print("<td><input type='submit' name='Order' value='UpdateShipment' class='btnbuy'></td>");
                        } else {
        ...
        if (request.getParameter("Order") != null && request.getParameter("Order").equals("UpdateShipment")) {
            try {
                MySqlDataStoreUtilities.updateOrder(1, 22, LocalDate.parse(request.getParameter("shipmentDate")));
